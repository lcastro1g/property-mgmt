...
/* 
 * Purpose: Search common page elements as PHP methods
...
function displayDashTable($searchType, $searchTerm) {
$tableHead = <<< TABLEHEAD
...
$tableRows = buildTableRows($searchType, $searchTerm);
 ...
 *----------------------------------------------------------------------------*/
function buildTableRows($searchType, $searchTerm) {
    ...
    switch($searchType) {
        case "status":
          $serviceRequests = getServiceRequestByRequestStatus($searchTerm);
          break;
        case "service":
          $serviceRequests = getServiceRequestByServiceArea($searchTerm);
          break; 
        default:
...
// Filter requests displayed on dasboard based on user role
        ...
        $rows .= <<< HTML
          <tr>
            ...
            <td><a href="./requestViewDetails.php?reqid=$ServiceRequestID" class="btn btn-success" role="button">Details</a>&nbsp<a href="./requestMaint.php?reqid=$ServiceRequestID" class="btn btn-success" role="button">Edit</a>&nbsp&nbsp$btnDeleteHTML</td>
          </tr>
HTML;
...
/* Customized search */
function displaySearchPanel($searchType, $searchTerm) {
    
    $resultString = "Showing results for: ";
    if($searchType == "all") {
        $resultString .= "ALL (no search applied)";
    } else {
        $resultString .= "search type = $searchType, search term = '$searchTerm'";
    }
